...
def build_map(game_state):
    ...
    grid = [ [SPACE for col in range(game_state['board']['height'])] for row in range(game_state['board']['width'])]
    turn = game_state['turn']
...
def closest_food(grid, game_state):
    my_location = get_coords(game_state['you']['head'])
    close_food = None
    close_distance = board_width * board_height
...
def astar(game_state, grid, destination):
    ...
    start = get_coords(game_state['you']['head'])
...
def build_astar_grid(game_state, grid):
...
def valid_move(d, grid, game_state):
    global board_height, board_width
    current = get_coords(game_state['you']['head'])
...
def best_move(recommended_move, data, grid):
    global board_height, board_width
    ...
    if kill_moves:
        ...
        if valid_move(recommended_move, grid, data):
            best_move = recommended_move
