...
tree_nodes = []
tree_node_counter = 1
# Random variables for timing runtime
tot_time_graph = 0
counter_graph = 0
copy_graph = 0
...
class Battlesnake:
    ...
    def update_board(self):
        ...
        for num, pos in enumerate(self.you.body):
            self.board[pos.x, pos.y] = "£" if num == 0 else "o"
    ...
    def closest_distance(self, start: Pos, end: Pos) -> int:
    ...
    @staticmethod
    def manhattan_distance(start: Pos, end: Pos) -> int:
        """
        ...
        """
        return sum(abs(value1 - value2) for value1, value2 in zip(start.as_tuple(), end.as_tuple()))

    def dijkstra_shortest_path(self, start: Pos, end: Pos) -> int:
        """
    ...
    @staticmethod
    def check_missing_nodes(G, nodes: list[tuple]):
        global copy_graph
        clock_in = time.time_ns()
    ...
    def is_move_safe(
        ...
        :param move: The location of the snake's hypothetical head as a Pos object e.g. Pos({"x": 5, "y": 10})
    ...
    def is_game_over(self, for_snake_id: str | list, depth: Optional[int] = None) -> tuple[bool, bool]:
        ...
        if isinstance(for_snake_id, (list, tuple)):
            ...
            snake_still_alive = snake_monitor[self.you.id] if for_snake_id is None else snake_monitor[for_snake_id]
    ...
    def simulate_move(self, move_dict: dict, evaluate_deaths: Optional[bool] = False) -> Battlesnake:
        ...
        board = {
            ...
            "food": [xy.as_dict() for xy in self.food],
    ...
    def dist_to_nearest_food(self) -> int:
        ...
        for food in self.food:
            ...
            dist_enemy = min([self.dijkstra_shortest_path(food, snake.head) if snake.length < self.you.length
    ...
    def heuristic(self, depth_number: int) -> tuple[float, dict]:
        """
        Evaluate the current board for our snake (the higher, the better)
        ...
        available_peripheral = self.flood_fill(self.you.id, confined_area="auto")
    ...
    def minimax(self, depth, alpha, beta, maximising_snake):
        ...
        # Minimax on opponent snakes
            ...
            for opp_id, opp_snake in self.opponents.items():
                ...
                dist_opp_to_us = self.manhattan_distance(self.you.head, opp_snake.head)
